...
def main():
    parser = argparse.ArgumentParser(description="Small Tiny-K pretraining")
    parser.add_argument("--out_dir", type=str, default="base_model_small")
    parser.add_argument("--data_path", type=str, default="./seq_monkey_learnllm_pretrain.jsonl")
    ...
    for epoch in range(args.epochs):
        for step, (X, Y, loss_mask) in enumerate(train_loader):
            ...
            if (global_step + 1) % args.save_interval == 0:
                ckp = f"{args.out_dir}/pretrain_{lm_config.dim}_{lm_config.n_layers}_{lm_config.vocab_size}.pth"
    ...
    ckp = f"{args.out_dir}/pretrain_{lm_config.dim}_{lm_config.n_layers}_{lm_config.vocab_size}.pth"
    torch.save(model.state_dict(), ckp)
    print(f"Saved pretrain checkpoint to {ckp}")
