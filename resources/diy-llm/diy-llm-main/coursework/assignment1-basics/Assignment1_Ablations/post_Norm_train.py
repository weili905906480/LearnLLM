...
def save_ppl_curve(train_ppls, val_ppls, save_dir):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "train_ppl2.png"))
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "val_ppl2.png"))
    plt.close()
...
def main():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./Assignment1_Ablations/post_Norm")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 1 == 0:
            ...
            save_checkpoint(
                path=f"./Assignment1_Ablations/post_Norm/epoch_{epoch}.pt",
                model=model,
                optimizer=optimizer,
                iteration=step_t,
                epoch=epoch,
