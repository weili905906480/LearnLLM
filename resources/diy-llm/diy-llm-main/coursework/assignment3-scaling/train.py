...
from scaling import estimate_flops
...
def main():
    ...
    no_embd , embd = compute_params_from_config(config)
    print(f"模型参数量: {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M")
    print(f"非嵌入参数: {no_embd}")
    print(f"嵌入参数: {embd}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            # 200step记录一次，不然容易记录噪声
            if step_t % 200 == 0:
                tokens_seen = step_t * args.batch_size * args.context_length
                flops = estimate_flops(no_embd, tokens_seen)
        ...
        np.save(
            os.path.join(args.checkpoint_dir, "scaling_records_70M.npy"),
            scaling_records
        )
